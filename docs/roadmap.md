...
| 20 | ✅ | NVIDIA Rubin preset (2026/2027 generation) — 288 GB HBM4 at 22 TB/s, 17.5 PFLOPS FP8 dense, 50 PFLOPS sparse NVFP4, NVLink 6 at 3.6 TB/s per chip. Per-chip specs from nvidia.com/en-us/data-center/hgx/. `--gpu rubin --tp 8` models one HGX Rubin NVL8 server. |
| 21 | ✅ | Mixed-precision quantization — `weight_bits`, `activation_bits`, `kv_bits`, `flops_fp4`, and `supports_2to4_sparsity` unlock FP4/NVFP4, W4A16, W4A8KV4, and sparse 2:4 configs. Adds quantized Llama/Kimi presets, roofline tests, [`docs/quantization.md`](quantization.md), and B200 FP4 validation against public TensorRT-LLM results in [`docs/quantization-validation.md`](quantization-validation.md). |
| 22 | ✅ | Scale-out networking — optional Ethernet / InfiniBand tier for multi-node TP, PP, and EP. Adds `--gpus-per-node`, `--scale-out-bw-gbps`, `--scale-out-latency-us`, and named fabric presets for HDR-200, NDR-400, Quantum-X800 / XDR-800, Spectrum-X400, and Spectrum-X800. See [`docs/topology.md`](topology.md). |
| 23 | ✅ | NVIDIA H200 and DGX system presets — `--gpu h200` models Hopper HBM3e (141 GB, 4.8 TB/s) with H100-class compute; `--system dgx-h100`, `dgx-h200`, and `dgx-b200` resolve 8-GPU NVLink/NVSwitch nodes with NDR-class scale-out defaults. |
| 24 | ✅ | Study report pipeline — `--study-models`, `--study-gpus`, `--study-systems`, `--study-tps`, `--study-arrival-rates`, `--json-out`, and `--html` generate comparison datasets and local HTML reports. Curated JSON datasets live in `reports/curated/`. |
| 25 | ✅ | GitHub Pages dashboard — `reports/dashboard.html` is the single public report UI, backed by `reports/manifest.json` and curated JSON result files. Root `index.html` redirects to the dashboard so the Pages site opens directly on the filterable results view. |

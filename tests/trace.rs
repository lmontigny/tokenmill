...
fn native_csv_loads() {
    let csv = "timestamp_ms,prompt_tokens,output_tokens\n\
               ...
               1000.0,1024,64\n\
               2500.5,256,32\n";
    let path = write_temp("inference_sim_test_native.csv", csv);
...
fn azure_csv_loads_and_normalises() {
    ...
    let path = write_temp("inference_sim_test_azure.csv", csv);
    let trace = TraceReplay::from_csv(&path).unwrap();
...
fn trace_skips_comment_lines() {
    let csv = "# this is a comment\n\
               ...
               # so is this\n\
               0.0,512,128\n";
    let path = write_temp("inference_sim_test_comments.csv", csv);
    let trace = TraceReplay::from_csv(&path).unwrap();
...
fn empty_trace_loads() {
    let csv = "timestamp_ms,prompt_tokens,output_tokens\n";
    let path = write_temp("inference_sim_test_empty.csv", csv);
    let trace = TraceReplay::from_csv(&path).unwrap();

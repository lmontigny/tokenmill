...
fn summary_carries_energy_when_tdp_is_set() {
    ...
    let s = m.to_summary("m", "g", "sched", 1, 1, false, 5.0, "roofline", 700.0);
    assert!(
...
fn summary_zero_energy_when_tdp_is_zero() {
    ...
    let s = m.to_summary("m", "g", "sched", 1, 1, false, 5.0, "roofline", 0.0);
...
fn disaggregated_doubles_chip_count_for_idle_accounting() {
    ...
    let s = m.to_summary("m", "g", "sched", 4, 1, true, 0.0, "roofline", 1000.0);
    assert!(
